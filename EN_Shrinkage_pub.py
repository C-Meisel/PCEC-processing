...
matter most in predicting negatrode shrinkage from processing parameters.
...
SSP_lb = '/Library/Fonts/SourceSansPro-SemiBold.ttf'
...
' ------ Lines to Edit'
# - Hyperparameters
alpha = 0.00304 # 0.00304
l1_ratio = 0.8213 # 0.8213
...
' ----- End Lines to Edit'
data = 'path_to_data' #
df = pd.read_excel(data,sheet_name='Shrinkage')
...
if datapoints == True:
    print('Number of datapoints:', len(df['Shrinkage']))

' ----- Elastic Net Model'
target = 'Shrinkage' # Electrolyte grain size (μm) Shrinkage
...
if tune_hyperparameters == True:
    def objective(trial):
        alpha = trial.suggest_float('alpha', a_min, a_max) # init: 1e-5, 1e2)
        l1_ratio = trial.suggest_float('l1_ratio', il_min, il_max) # init: 0.0, 1.0
...
if plot_coefs == True:
    ...
    barplot = sns.barplot(data=df,x='Feature',y='Coefficient',palette=palette)

    # --- formatting and displaying
    ax.set_xlabel('Feature',size='x-large')
    ax.set_ylabel('Coefficient (a.u.)',size='xx-large', labelpad=10)
...
if plot_all_residuals == True:
    ...
    # Calculate the Mean Squared Error (MSE) from residuals
    mse_cv = np.mean(residuals**2)
    print(f'Mean Squared Error (MSE) for all CV: {mse_cv}')
...
if plot_coefficients_residuals == True:
    ...
    def adjust_lightness(color, lightness_factor):
        ...
        # Adjust the lightness by interpolating between white (lightness=1) and the original color
    ...
    for i, patch in enumerate(barplot.patches):
        # Get the feature's overall category
        overall_category = df['Overall_Category'].iloc[i]
        base_color = palette_map[overall_category]# Get the base color from the palette map

        min_lightness = 0.7 # Adjust hte minimum lightness for the colorbars
        lightness_factor = norm(abs(df['Coefficient'].iloc[i])) * (1-min_lightness) + min_lightness  # Scale to 0.75 - 1.0
        adjusted_color = adjust_lightness(base_color, lightness_factor)  # Get the base color from the palette map
    ...
    # ax1.axhline(y=0, color='black', linestyle=(0, (4, 6.4)), linewidth=1)
